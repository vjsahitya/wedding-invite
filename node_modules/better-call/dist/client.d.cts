...
declare const createClient: <R extends Router | Router["endpoints"], GlobalOpts extends ClientOptions = ClientOptions>(options?: GlobalOpts) => <OPT extends UnionToIntersection<InferClientRoutes<R extends {
  endpoints: Record<string, Endpoint>;
} ? R["endpoints"] : R> extends {
  [key: string]: infer T_1;
} ? T_1 extends Endpoint ? { [key in T_1["options"]["method"] extends "GET" ? T_1["path"] : `@${T_1["options"]["method"] extends string ? Lowercase<T_1["options"]["method"]> : never}${T_1["path"]}`]: T_1; } : {} : {}> extends (infer T) ? { [K_1 in keyof T]: T[K_1]; } : never, K extends keyof OPT, C extends InferContext<OPT[K]>, CallOpts extends BetterFetchOption<C["body"], C["query"], C["params"]>>(path: K, ...callOptions: HasRequired<C> extends true ? [WithRequired<BetterFetchOption<C["body"], C["query"], C["params"]>, keyof RequiredOptionKeys<C>>] : [CallOpts?]) => Promise<BetterFetchResponse<Awaited<ReturnType<OPT[K] extends Endpoint ? OPT[K] : never>>, unknown, CallOpts["throw"] extends boolean ? CallOpts["throw"] : GlobalOpts["throw"] extends true ? true : false>>;
//#endregion
