	...
	constructor(options, getStoreConfig) {
		...
		this.update = (newOptions) => {
			...
			if (this.options.routeTree !== this.routeTree) {
				...
				if (process.env.NODE_ENV !== "development" && (_tanstack_router_core_isServer.isServer ?? this.isServer) && globalThis.__TSR_CACHE__ && globalThis.__TSR_CACHE__.routeTree === this.routeTree) {
					const cached = globalThis.__TSR_CACHE__;
					this.resolvePathCache = cached.resolvePathCache;
					processRouteTreeResult = cached.processRouteTreeResult;
				} else {
					this.resolvePathCache = require_sieve_cache.createSieveCache(1e3);
		...
		this.navigate = async ({ to, reloadDocument, href, publicHref, ...rest }) => {
			const hrefIsUrl = !!href && require_utils.isAbsoluteUrl(`${href}`);
