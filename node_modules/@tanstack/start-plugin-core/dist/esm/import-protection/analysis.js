...
function buildImportAnalysis(result) {
	...
	const getBindingInfo = (source) => getOrCreate(importBindingsBySource, source, () => ({ importedLocalNames: /* @__PURE__ */ new Set() }));
		...
		if (t.isImportDeclaration(node)) {
			...
			if (t.isIdentifier(object)) {
				...
				if (sources) {
					...
					for (const source of sources) if (!node.computed && t.isIdentifier(property)) addMockName(source, property.name);
...
//#endregion
export { findOriginalUnsafeUsagePosFromResult, findPostCompileUsagePosFromResult, getImportSourcesFromResult, getImportSpecifierLocationFromResult, getMockExportNamesBySource, getMockExportNamesBySourceFromResult, getNamedExports, getNamedExportsFromResult, isValidExportName };
